...
export const uploadFilesToS3AndUpdateDbUrl = async (id_urls) => {
    ...
    const insert_to_hasura_tagging = id_urls.map((tagging_insert_info, i) => async () => {
        try {
            ...
            const png_convert_url="http://styleclueless.com:3001/removebg/?s3_path="+s3_filename;
            console.log(`${i }  - ${png_convert_url } converting to transparent png for ${s3_filename}` )
